...
function Sheep:LoadTrack(name, animId, priority, looped)
	...
	if not ok or not track then
		warn("[Sheep] No se pudo cargar animaciÃ³n:", name, animId)
		return nil
	end
...
function Sheep:PlaySequenceLoop(sequenceName, token)
	...
	if not loopName then
		warn("[Sheep] No hay loops vÃ¡lidos para secuencia:", sequenceName)
		return
	end
...
function Sheep:PlaySequenceAction(sequenceName)
	...
	if #validLoops == 0 then
		warn("[Sheep] No se puede iniciar secuencia sin loop vÃ¡lido:", sequenceName)
...
function Sheep:WarnInvalidActionOnce(actionName, reason)
	...
	warn("[Sheep] AcciÃ³n invÃ¡lida ignorada:", actionName, "-", reason or "sin razÃ³n", "en", self.Model and self.Model.Name or "oveja")
end
...
function Sheep:IsSequencePlayable(sequenceName)
	...
	if #validLoops == 0 then
		return false, "no tiene loops vÃ¡lidos"
	end
	...
	if sequenceData.ExitBeforeMove ~= false and not hasEnd then
		return false, "falta End o FallbackEnd vÃ¡lido"
	end
...
-- =========================================================================
-- IA: reacciÃ³n, calma, flow y pÃ©rdida
-- =========================================================================
